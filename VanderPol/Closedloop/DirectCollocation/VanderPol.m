function [problem,guess] = VanderPol
%VanderPol - Van der Pol Problem in Closed-loop
%
% Syntax:  [problem,guess] = VanderPol
%
% Outputs:
...
InternalDynamics=@VanderPol_Dynamics_Internal;
SimDynamics=@VanderPol_Dynamics_Sim;
...
% Settings file
problem.settings=@settings_VanderPol;

problem.FcnTypes.StageCost='Nonlinear';
problem.FcnTypes.TerminalCost='None';
...
problem.states.x0=[0.75 0.75];

% Initial conditions for system. Bounds if x0 is free s.t. x0l=< x0 <=x0u
problem.states.x0l=[0.75 0.75]; 
problem.states.x0u=[0.75 0.75]; 
...
guess.states(:,1)=[0.2 -0.15];
guess.states(:,2)=[0 -0.13];
...
e1=x(:,1)-0.2*cos(-(vdat.ini_t+t));
e2=x(:,2)-0.2*sin(-(vdat.ini_t+t));


stageCost = e1.*e1+e2.*e2;
% stageCost = e1.*e1; % change made by nikilesh on 22 Jan, checking if having two error makes a difference: YES, it does make a diff but why?
