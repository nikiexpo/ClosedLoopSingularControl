...
solutionOL_IRR.t = t_sol;
solutionOL_IRR.x = x_sol;
solutionOL_IRR.u = u_sol;
% figure
% subplot(3,1,1)
% hold on
...
legend({'$x_1$','$x_{1_{ref}}$'}, Interpreter='latex', FontSize=10)
xlabel('Time [s]', Interpreter='latex', FontSize=10)
ylabel('States', Interpreter='latex', FontSize=10)
ylim([-1 1])
xlim([0 solution.tf])
grid on
...
legend({'$x_2$','$x_{2_{ref}}$'}, Interpreter='latex', FontSize=10)
xlabel('Time [s]', Interpreter='latex', FontSize=10)
ylabel('States', Interpreter='latex', FontSize=10)
ylim([-1 1])
xlim([0 solution.tf])
...
ylabel('Tracking Error', Interpreter='latex', FontSize=10)
...
legend({'Controlled','Reference'}, Interpreter='latex', FontSize=10)
xlabel('State $x_1$', Interpreter='latex', FontSize=10)
ylabel('State $x_2$', Interpreter='latex', FontSize=10)
grid on
...
% a = 0.2;
% b = -1;
% u_pmp = x_sol(:,1).*(1+c1.*x_sol(:,2) + c3) - c1.*c2.*x_sol(:,2) - a.*cos(b.*t_sol) + x_sol(:,1)./(x_sol(:,2)) .*a.*sin(b.*t_sol);
% plot(t_sol, u_pmp)
% hold on
% plot(t_sol,u_sol(:,1) )
% hold off
% legend(["u_analytic", "u_sol"])
% ylim([-2,2])