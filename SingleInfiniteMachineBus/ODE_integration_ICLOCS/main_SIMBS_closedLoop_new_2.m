...
for i=1:t_end/t_step
    ...
    if (all(speval(solution,'U',1,0:sim_step:t_step)<0.99) && ~isempty(x_sol)) || i_flag
        opts = odeset(RelTol=1e-9);
        [ tv, xv ] = ode113(@(t,x) SIMBS_Dynamics_Sim2(x, t, problem.data)', 0:sim_step:t_step, xv(end,:),opts);
        vdat=problem.data;
        x1=xv(:,1);
        x2=xv(:,2);
