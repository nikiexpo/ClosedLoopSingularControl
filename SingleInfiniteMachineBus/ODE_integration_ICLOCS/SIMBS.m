function [problem,guess] = SIMBS
%SIMBS - Single Infinite Machine Bus Problem
%
% Syntax:  [problem,guess] = SIMBS
%
% Outputs:
...
InternalDynamics=@SIMBS_Dynamics_Internal;
SimDynamics=@SIMBS_Dynamics_Sim;
...
% Settings file
problem.settings=@settings_SIMBS;
...
problem.time.tf_min=0.5;     
problem.time.tf_max=0.5; 
guess.tf=0.5;
...
problem.states.x0=[1.5 15];

% Initial conditions for system. Bounds if x0 is free s.t. x0l=< x0 <=x0u
problem.states.x0l=[1.5 15]; 
problem.states.x0u=[1.5 15]; 
...
problem.data.H=0.0106;
problem.data.Xt=0.28;
problem.data.Pm=1;
problem.data.Es=1.21;
problem.data.V=1;
problem.data.Pe=problem.data.Es*problem.data.V/problem.data.Pm/problem.data.Xt;
problem.data.D=0.03;
problem.data.dep=asin(1/problem.data.Pe);
problem.data.C1=3;
problem.data.C2=30;
