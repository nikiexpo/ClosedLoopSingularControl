...
for i=1:t_end/t_step
    ...
    if (all(speval(solution,'U',1,0:sim_step:t_step)<0.99) && ~isempty(x_sol)) 
        opts = odeset(RelTol=1e-6);
        problem.data.solution=solution;
        [ tv, xv ] = ode113(@(t,x) SIMBS_Dynamics_Sim1(x, t, problem.data)', 0:sim_step:t_step, xv(end,:),opts);
        vdat=problem.data;
        x1=xv(:,1);
        x2=xv(:,2);
        uv=-(vdat.C1^2.*vdat.D.*x2 - vdat.C1^2*vdat.Pm + 2.*vdat.C1^2.*vdat.H.*x2 + 2.*vdat.C2^2.*vdat.H.*x1)./(vdat.C1^2.*vdat.Pe.*sin(vdat.dep + x1));
        t_sol_an=[t_sol_an;t_sol(end)+tv(1:end-1)];
        u_sol_an=[u_sol_an;uv(1:end-1,:)];
        x_sol_an=[x_sol_an;xv(1:end-1,:)];
    else
...
hold on
plot(t_sol(1:end-1),u_sol(:,1) )
scatter(t_sol_an,u_sol_an(:,1),'LineWidth',2 )
hold off
grid on
