...
for i=1:t_end/t_step
    ...
    if i==1
        ...
        solution_mod.x0=[solution.x0;0];
    else
        vdat.ini_t=t_update(i);
        [OCP] = updateMyProblem(OCP,'z0',solution.z,'x0',xv(end,1:end-1),'userdata',vdat); % look into the OCP structure
        [solution,MRHistory,OCP]=solveMyProblem(problem,guess,options,OCP);
        solution_mod=solution;
        solution_mod.x0=[solution.x0;xv(end,end)];
    end
    
    [ tv, xv, uv ] = simulateSolutionSegment( problem, solution_mod, 'ode113', 0:sim_step:t_step );
    t_sol=[t_sol;t_sol(end)+tv(2:end)];
    u_sol=[u_sol;uv(1:end-1,:)];
    x_sol=[x_sol;xv(1:end-1,1:end-1)];
    cost_sol=[cost_sol;xv(1:end-1,end)];
    comp_time=[comp_time;MRHistory.cpu];
    % p2_sol = [p2_sol; solution.multipliers.lambda_1toN(:,2)];
end
x_sol(end+1,:)=xv(end,1:end-1);
cost_sol(end+1,end)=cost_sol(end,end);
%%
figure
subplot(2,1,1)
hold on
